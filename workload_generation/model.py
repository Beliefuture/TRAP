...
from workload_generation.generation_utils import constants, mod_sql
...
class SelfCriticCriterion(nn.Module):
    ...
    def forward(self, samlog_props, sample_words,
        ...
        :return:
        """
        # advantage: torch.Size([16])
        # clamp: confirm the min value <-> eps? 16
        try:
            # todo: newly added.
            if advantage.size(0) > 1:
                advantage = (advantage - advantage.mean()) / advantage.std().clamp(min=1e-8)
        except:
            print("advantage calculation error!")
        # tgt/mask: 0/1, torch.Size([16, 17])
        mask = (tgt_seq > 0).float()

        # fetch the value according to the index.
        # torch.Size([16, 17, 3040]) -> torch.Size([16, 17, 1]) -> torch.Size([16, 17])
        # todo: newly added.
        # s_props = samlog_props.gather(2, sample_words.unsqueeze(2)).squeeze()
        ...
        # advantage: torch.Size([16]) -> torch.Size([16, 1]) -> torch.Size([16, 17])
        advantage = advantage.unsqueeze(1).repeat(1, mask.size(1))
        advantage = advantage.detach()

        # average the loss per word.
        # additional `* advantage` item compared with the `CrossEntropy`.
        # s_props/mask/advantage: torch.Size([16, 17])
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_word, hidden):
        ...
        # initial decoder hidden is final/last hidden state of
        # the forwards(-2) and backwards(-1) encoder RNNs fed through a linear layer.
        if self.is_bid:
...
class Attention(nn.Module):
    ...
    def forward(self, src_tensor, encoder_outputs, decoder_hidden, inf):
        ...
        3. concat: {v_a}^T * tanh(W_a * [h_t;h_s])
        ...
        # decoder_hidden = [batch size, src len, dec hid dim]
...
class DecoderRNN(nn.Module):
    def __init__(self, output_size, emb_size, enc_hidden_size, dec_hidden_size,
        ...
        # (output: dec_hidden_size, context: enc_hidden_size * 1/2, embedded: dec_hidden_size)
    ...
    def init_hidden(self, batch_size, device):
        # todo: useless? (D∗num_layers, N, H_out)
    ...
    def forward(self, word_input, decoder_hidden, device,
        ...
        embedded = self.dropout(self.embedding(word_input))  # F.relu()?

        rnn_input = embedded
        if self.attention is not None:
            # todo: newly added for attention masking.
        ...
        # torch.save(output, "./fc_output.pt")  # todo: to be removed.
        if valid_mask is not None:
            output = torch.where(valid_mask.unsqueeze(1), torch.tensor(-inf).to(device), output)
            # torch.save(output, "./mask_fc_output.pt")  # todo: to be removed.
        ...
        if self.is_ptr:
            ...
            if valid_mask is not None:
                ...
                # torch.save(valid_mask, "./valid_mask.pt")  # todo: to be removed.
                # torch.save(src_tensor[:, step], "./src_tensor_step.pt")  # todo: to be removed.
                # torch.save(step_valid, "./step_valid.pt")  # todo: to be removed.
                # torch.save(is_not_pad, "./is_not_pad.pt")  # todo: to be removed.
            ...
            if valid_mask is None:
                ...
                # output.scatter_add_(1, word_input, ptr_output[:, decode_step].unsqueeze(-1))
            # ii) only corresponding current step encoder word input
            else:  # invalid -> prob_ptr -> 0
                # ptr_output = prob_ptr * torch.where(torch.gather(valid_mask, 1, src_tensor[:, step].unsqueeze(-1)), 0,
                #                                     torch.ones(word_input.shape, device=device, dtype=torch.int64))

                # ptr_output = (scores.squeeze(1)[:, step]).unsqueeze(-1)
                # ptr_output = prob_ptr * ptr_output
                # ptr_output = ptr_output * torch.where(torch.gather(valid_mask, 1, src_tensor[:, step].unsqueeze(-1)), 0,
                #                                       torch.ones(word_input.shape, device=device, dtype=torch.int64))

                # output.scatter_add_(1, src_tensor[:, step].unsqueeze(-1), ptr_output)
                # output = output.unsqueeze(1)
                ...
                # torch.save(output, "./nomask_output.pt")

                # set the probability of the corresponding invalid token to 0.
                output = torch.where(valid_mask.unsqueeze(1), torch.tensor(0.).to(device), output)

                # torch.save(scores, "./scores.pt")
                # torch.save(prob_ptr, "./prob_ptr.pt")
                # torch.save(ptr_output, "./ptr_output.pt")
                # torch.save(prob_gen, "./prob_gen.pt")
                # torch.save(gen_output, "./gen_output.pt")
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src_tensor, tgt_tensor, device,
        ...
        decoder_input = torch.tensor([[constants.BOS]] * batch_size, device=device)  # torch.Size([64, 1])
        decoder_hidden = encoder_hidden  # torch.Size([64, 1, 256]), torch.Size([64, 128])
        ...
        decoder_outputs, decoded_words = list(), list()  # for the tgt_seq
        ...
        for di in range(self.max_len):
            ...
            if sql_tokens is not None:
                ...
                # stack them
                index_data = torch.stack(index_data)  # .repeat(int(batch_size / len(word_cand)), 1)
            ...
            if self.args.is_attn:
                decoder_output, decoder_hidden, decoder_attention = self.decoder(
                    ...
                    encoder_outputs, src_tensor, di,  # attn/ptr
            ...
            if sql_tokens is not None and self.args.pert_mode == "column":
                for bno, (word, token) in enumerate(zip(decoder_input, sql_tokens)):
                    ...
                    if token["pre_types"][di] == "select#column" or \
                        ...
                        assert word in token["pno_tokens"], "Error! New column occurred during perturbation!"
                        # assert word in token["pcolumn_vecs"], "Error! New column occurred during perturbation!"
...
class SingleGRUModel(nn.Module):
    ...
    def attention(self, output, hidden):
        ...
        # hidden = hidden.view(-1, n_hidden * 2, 1)
        hidden = hidden.view(output.shape[0], -1, 1)
        # attn_weights : [batch_size, n_step]
        attn_weights = torch.bmm(output, hidden).squeeze(2)
        soft_attn_weights = F.softmax(attn_weights, dim=1)

        # [batch_size, n_hidden * num_directions(=2), n_step] *
        # [batch_size, n_step, 1] = [batch_size, n_hidden * num_directions(=2), 1]
    ...
    def forward(self, src_tensor, device,
        ...
        rnn_outputs = list()
        decoded_outputs, decoded_words = list(), list()  # for the tgt_seq
        for di in range(self.max_len):
            valid_mask = None
            if sql_tokens is not None:
                batch_max_len = max([len(sql_token["pre_types"]) for sql_token in sql_tokens])
            ...
            if self.args.is_attn:
                # output, _ = self.attention(output, hidden)
            ...
            if max_props:
                # values: torch.Size([16, 1]), indices: torch.Size([16, 1])
        ...
        if max_props:
            ...
            # torch.Size([16, 17]), torch.Size([16, 17, 9049])
